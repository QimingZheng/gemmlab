...
template <typename T>
void ParallelKernel<T>::Call(T* C, const T* A, const T* B, int M, int N,
                             int K) {
  const int iTile = 32, jTile = 32, kTile = 16;
  const int iOuterBound = M / iTile, jOuterBound = N / jTile,
            kOuterBound = K / kTile;

  __m256 c, a, b;
  const int floatNum = 256 / 32;
  ...
  {
#pragma omp parallel for shared(A, C) private(a, b, c) num_threads(32)
        ...
        for (int k_outer = 0; k_outer < kOuterBound; k_outer++) {
          for (int i_inner = 0; i_inner < iTile; i_inner++) {
            for (int k_inner = 0; k_inner < kTile; k_inner++) {
              ...
              for (int _j_inner = 0; _j_inner < jTile / floatNum; _j_inner++) {
                b = _mm256_load_ps(B + j_outer * K * jTile +
                                   k_outer * jTile * kTile + k_inner * jTile +
                                   _j_inner * floatNum);
                c = _mm256_mul_ps(b, a);
...
template class ParallelKernel<float>;
